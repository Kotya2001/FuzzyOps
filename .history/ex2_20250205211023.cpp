...
double calculate_final_scores(PyObject *pDomain, const char *name1, const char *name2) {
    ...
    PyList_SetItem(
        pNumList, 0, pAttrib1); // Важно: PyList_SetItem не требует ручного освобождения pAttrib1
    PyList_SetItem(pNumList, 1, pAttrib2); // То же самое для pAttrib2

    PyObject *pResult = PyObject_CallObject(pCalcMethod, pNumList);
    if (!pResult) {
        PyErr_Print();
    ...
    Py_DECREF(pNumList); // Освобождаем список
